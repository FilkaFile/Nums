...
def create_item(data: Item228 = Body(description="Создаём поля нового объекта:")) -> str:
...
def get_item(id: str = Path(description="айдишник получаемого предмета")) -> str:
    item = find_item(id)
    if item == None:
        return JSONResponse(
...
def delete_item(id: str = Path(description="айдишник удаляемого предмета")) -> str:
    item = find_item(id)
    if item == None:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
...
def edit_item(id: str = Path(description="айдишник изменяемого предмета"), data: Item228 = Body(description="Изменяем поля объекта:")) -> str:
    item = find_item(id)
    if item == None:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
